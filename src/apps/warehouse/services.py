...
async def get_single_warehouse(
    ...
    if not (
        warehouse_object := await if_exists(Warehouse, "id", warehouse_id, session)
    ):
...
async def manage_warehouse_state(
    ...
    if max_waiting_rooms is not None:
        new_available_waiting_rooms = (
            max_waiting_rooms - warehouse_object.occupied_waiting_rooms
        )
