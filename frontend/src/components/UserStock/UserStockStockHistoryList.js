...
const UserStockStockHistoryList = ({ themeMode }) => {
    ...
    const fetchUserStocks = async (page, size) => {
        try {
            setLoading(true);
            const response = await axios.get(`http://localhost:8000/api/stocks/all/${stockId}/history?page=${page}&size=${size}`, {
                headers: {
...
export default UserStockStockHistoryList;
