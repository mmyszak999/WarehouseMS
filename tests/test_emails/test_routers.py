...
from src.apps.users.schemas import UserLoginInputSchema, UserOutputSchema, UserInputSchema, UserPasswordSchema
...
async def test_user_can_succesfully_activate_their_account_via_activation_link(
    ...
    response = await async_client.post(
        f"email/confirm-account-activation/{token}",
        content=user_passwords.json())
