...
from src.core.factory.stock_factory import (
    StockInputSchemaFactory
)
...
async def test_if_stocks_were_created_correctly(
    ...
    product_count = 5
    stock_inputs = [StockInputSchemaFactory().generate(
        product_weight=product.weight,
        product_count=product_count,
        product_id=product.id) for product in db_products.results]

    stocks = await create_stocks(async_session, testing=True, input_schemas=stock_inputs)
    
    assert {product_count} == {stock.product_count for stock in stocks}
    assert {product.id for product in db_products.results} == {stock.product_id for stock in stocks}
...
async def test_if_stocks_are_issued_correctly(
    ...
    assert len(issued_stocks) == len({stock for stock in issued_stocks if stock.is_issued})
