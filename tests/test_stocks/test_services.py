...
async def test_raise_exception_when_there_is_no_waiting_room_available_for_new_stocks(
    ...
    products = [await if_exists(Product, "id", db_products.results[0].id, async_session)]
    product_counts = [4]
...
async def test_if_stocks_are_issued_correctly(
    ...
    old_waiting_room = await if_exists(WaitingRoom, "id", stock_object.waiting_room_id, async_session)
