...
async def test_if_rack_level_max_slots_are_correctly_managed(
    ...
    rack_level = await if_exists(RackLevel, "id", db_rack_levels.results[0].id, async_session)
...
async def test_if_rack_level_weight_limits_are_correctly_managed(
    ...
    assert rack_level.available_weight == db_rack_levels.results[0].available_weight + weight
    assert rack_level.occupied_weight == db_rack_levels.results[0].occupied_weight - weight
...
async def test_if_rack_level_limits_are_correctly_managed_when_changing_max_weight_and_slots(
    ...
    rack_level_object = await if_exists(RackLevel, "id", rack_level_output.id, async_session)
...
async def test_raise_exception_when_new_max_weight_smaller_than_occupied_weight_amount_when_updating_rack_level(
    ...
    with pytest.raises(TooLittleWeightAmountException):
        await update_single_rack_level(async_session, rack_level_input, rack_level_object.id)
...
async def test_check_if_rack_state_managed_correctly_when_rack_level_max_weight_updated(
    ...
    rack_level_output = await update_single_rack_level(async_session, rack_level_input_2, rack_level_object.id)
