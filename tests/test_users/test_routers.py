...
@pytest.mark.parametrize(
    ...
    [
        (pytest.lazy_fixture('db_user'), pytest.lazy_fixture('auth_headers'), status.HTTP_200_OK),
        (pytest.lazy_fixture('db_staff_user'), pytest.lazy_fixture('staff_auth_headers'), status.HTTP_200_OK)
    ],
)
@pytest.mark.asyncio
async def test_staff_and_authenticated_user_can_get_active_users(
    ...
    request: pytest.FixtureRequest, status_code: int
):
...
@pytest.mark.parametrize(
    "user, user_headers, status_code",
    [
        (pytest.lazy_fixture('db_user'), pytest.lazy_fixture('auth_headers'), status.HTTP_403_FORBIDDEN),
        (pytest.lazy_fixture('db_staff_user'), pytest.lazy_fixture('staff_auth_headers'), status.HTTP_200_OK),
...
async def test_only_staff_user_and_authenticated_user_can_get_single_user(
    async_client: AsyncClient, user: UserOutputSchema, user_headers: dict[str, str],
    request: pytest.FixtureRequest, status_code: int, db_user: UserOutputSchema
):
